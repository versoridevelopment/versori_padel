...
export async function POST(req: Request, { params }: { params: { id: string } }) {
  try {
    ...
    if (!id_cancha || !inicio || ![60, 90, 120].includes(duracion_min)) {
      return NextResponse.json({ error: "Template inválido (cancha/inicio/duración)" }, { status: 409 });
    }
    ...
    // segmento por tipo_turno (alineado con tu /api/admin/reservas)
    const segmento_override = tipo_turno === "profesor" ? "profe" : "publico";

    // última fecha ya generada
    ...
    // generamos weeks_ahead semanas desde start_from
    const dates: string[] = [];
    ...
    const conflicts: Array<{ fecha: string; inicio: string; fin: string; code: string }> = [];
    let created_count = 0;

    for (const fecha of dates) {
      // calcular precio (tu endpoint)
      ...
      if (!calcRes.ok || !calcJson?.ok) {
        // si no puede calcular, lo tratamos como conflicto “lógico”
      ...
      if (insErr) {
        const code = (insErr as any).code || "ERR";
        if (code === "23P01") {
