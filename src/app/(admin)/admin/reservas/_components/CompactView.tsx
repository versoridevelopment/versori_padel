...
const timeStringToDecimal = (
  ...
  if (!timeStr || typeof timeStr !== "string") return NaN; // Retorna NaN explícito si no es string

  const [hStr, mStr] = timeStr.split(":");
  const h = Number(hStr);
  const m = Number(mStr);

  if (isNaN(h) || isNaN(m)) return NaN; // Si no son números válidos
...
export default function CompactView({
  ...
  const getTopPosition = (startStr: string) => {
    ...
    if (isNaN(hours)) return GRID_TOP_OFFSET; // Fallback seguro
  ...
  const getHeight = (startStr: string, endStr: string) => {
    ...
    if (isNaN(startDec) || isNaN(endDec)) return 50; // Altura default por seguridad

    return (endDec - startDec) * PIXELS_PER_HOUR;
  };

  // ✅ CORREGIDO: getCierreStyle validando NaN
  const getCierreStyle = (inicioStr: string, finStr: string) => {
    ...
    // Si hay datos corruptos, no renderizamos el cierre
  ...
  // ✅ CORREGIDO: isSlotBlocked validando NaN
  const isSlotBlocked = (cancha: CanchaUI, timeDecimal: number) => {
    ...
    return cancha.cierres.some((cierre) => {
      ...
      if (isNaN(s) || isNaN(e)) return false; // Ignorar cierres mal formados
      ...
      <div className="flex-1 overflow-auto relative custom-scrollbar bg-slate-50/30">
        <div className="flex min-w-max" style={{ height: totalHeight }}>
          ...
          <div className="w-16 sticky left-0 z-30 bg-white border-r border-slate-200 flex-shrink-0">
            <div className="h-12 border-b border-slate-200 bg-slate-50/80 backdrop-blur-sm sticky top-0 z-40 flex items-center justify-center p-1">
              <button
                onClick={onRefresh}
                disabled={isLoading}
                className={`p-1.5 rounded-lg transition-all ${isLoading ? "animate-spin text-blue-500" : "text-slate-400 hover:text-blue-600"}`}
              >
                <RefreshCw size={16} />
              </button>
            </div>
          ...
          {canchas.map((cancha) => {
            ...
            // Filtro de reservas seguro: intenta usar las anidadas, sino las globales
              ...
              >
                ...
                <div className="relative w-full h-full bg-white group-hover/col:bg-slate-50/30 transition-colors">
                  ...
                  {cancha.cierres?.map((cierre) => {
                    ...
                    if (!style) return null; // Si style es null por NaN o rango, no renderiza
                    return (
                      <div
                  ...
                  {timeSlots.map((time) => {
                    if (time === endHour && !Number.isInteger(time))
                      return null;
                    if (isSlotBlocked(cancha, time)) return null; // 🚫 BLOQUEO
                    ...
                    return (
                      <div
                        ...
                        className={`absolute w-full cursor-pointer hover:bg-blue-50/50 transition-colors z-0 ${Number.isInteger(time) ? "border-b border-slate-200" : "border-b border-slate-100 border-dashed"}`}
                        style={{
                          top:
                  ...
                  {reservasCancha.map((reserva) => {
                    ...
                    return (
                      <div
                        ...
                        className={`absolute left-1 right-1 rounded-xl border-l-[4px] shadow-sm cursor-pointer z-20 hover:shadow-md hover:-translate-y-0.5 transition-all p-2 flex flex-col justify-between ${theme.bg} ${theme.border} bg-opacity-95 backdrop-blur-sm`}
                        style={{
                      ...
                      >
                        <div className="flex justify-between items-start">
                          ...
                          <div
                            className={`w-2 h-2 rounded-full ring-2 ring-white ${reserva.estado === "confirmada" ? "bg-emerald-500" : "bg-amber-400"}`}
                          />
                        </div>
                        ...
                        <div className="flex justify-between items-end">
                          <span
                            className={`text-[8px] px-1 rounded border uppercase ${tipoTurnoBadgeClass(reserva.tipo_turno)}`}
                          >
                            {tipoLabel}
                          </span>
