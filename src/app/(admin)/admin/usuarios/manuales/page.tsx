...
export default function UsuariosManualesPage() {
  ...
  const filteredAndSorted = useMemo(() => {
    ...
    result.sort((a, b) => {
      // Priorizar Activos arriba siempre
  ...
  return (
    <div
      className="flex-1 w-full bg-slate-50 min-h-screen p-6 md:p-10 space-y-8 font-sans"
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 border-b border-slate-200 pb-6">
        ...
        {!loading && clientes.length > 0 && (
          <div className="flex items-center gap-4 bg-white p-2 rounded-2xl border border-slate-200 shadow-sm">
            <div
              className={`px-4 py-2 ${userRole === "admin" ? "border-r border-slate-100" : ""}`}
            >
      ...
      <div className="flex flex-col md:flex-row gap-4 items-center">
        ...
        <div className="relative w-full md:w-auto">
          <button
            ...
            className="w-full md:w-auto px-5 py-3 bg-white border border-slate-200 rounded-xl shadow-sm flex items-center justify-between gap-3 font-semibold text-slate-700 hover:bg-slate-50 transition-all"
          >
            ...
            {showFilters && (
              ...
              >
                <div className="p-1.5 space-y-0.5">
                  ...
                  >
                    <Clock className="w-4 h-4 text-orange-500" /> Más Recientes
                  </button>
                  <button
                    onClick={() => {
                      setSortBy("frecuente");
                        ...
                        >
                          ...
                          {activeMenuId === cliente.id && (
                            <div className="absolute right-8 top-8 w-40 bg-white rounded-xl shadow-xl border border-slate-100 z-50 p-1 animate-in fade-in zoom-in-95 duration-100">
                              {/* ⚡ CAMBIO: Ahora redirige a la vista detallada */}
                              <button
                                onClick={(e) => {
                                  e.stopPropagation();
                                  router.push(
