...
export default function NosotrosPageForm({ clubId, initialData }: Props) {
  ...
  // 2. Home Nuevas (Sincronizado)
  const moveHomeNew = (index: number, direction: "left" | "right") => {
    // Verificar limites antes de ejecutar
    if (direction === "left" && index === 0) return;
    if (direction === "right" && index === newHomeSliderFiles.length - 1)
      return;

    setNewHomeSliderFiles((prev) => moveItem(prev, index, direction));
    setNewHomeSliderPreviews((prev) => moveItem(prev, index, direction));
  };
  ...
  // 4. Galería Nuevas (Sincronizado)
  const movePageNew = (index: number, direction: "left" | "right") => {
    if (direction === "left" && index === 0) return;
    if (direction === "right" && index === newGalleryFiles.length - 1) return;

    setNewGalleryFiles((prev) => moveItem(prev, index, direction));
    setNewGalleryPreviews((prev) => moveItem(prev, index, direction));
  };
  ...
  const addValor = () => {
    setData((prev) => ({
      ...
      valores: [
        ...prev.valores,
        { titulo: "Nuevo Valor", contenido: "Descripción..." },
      ],
    }));
  };
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 p-6 md:p-10 -m-6 md:-m-10">
      ...
      <div className="md:hidden fixed bottom-6 right-6 z-[9999]">
        <button
          ...
          onClick={handleSave}
          disabled={saving}
          className="bg-green-600 text-white w-14 h-14 rounded-full shadow-2xl flex items-center justify-center border-2 border-white/20 active:scale-95 transition-all"
        >
          {saving ? (
      ...
      <div className="max-w-7xl mx-auto space-y-8 pb-32">
        ...
        <div className="flex justify-between items-end">
          ...
          <button
            ...
            onClick={handleSave}
            disabled={saving}
            className="hidden md:flex bg-green-600 hover:bg-green-700 text-white px-8 py-3 rounded-xl font-bold gap-2 items-center shadow-lg disabled:opacity-70"
          >
            {saving ? (
        ...
        <div className="flex space-x-1 bg-white p-1 rounded-xl border border-slate-200 shadow-sm overflow-x-auto">
          {[
            ...
            <button
              ...
              className={`flex items-center gap-2 px-6 py-3 rounded-lg font-bold text-sm whitespace-nowrap transition-colors ${
                activeTab === tab.id
        ...
        <div className="space-y-6">
          <section className="bg-white p-4 md:p-6 rounded-2xl border border-slate-200 shadow-sm flex justify-between items-center">
            <div>
              <h3 className="font-bold text-slate-800">Estado Sección</h3>
            ...
            <button
              type="button"
              onClick={() =>
                setData((p) => ({ ...p, activo_nosotros: !p.activo_nosotros }))
              }
                  ...
                  <section className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
                    ...
                    <div className="border-t pt-6">
                      <div className="flex justify-between items-center mb-4">
                        ...
                        <label className="cursor-pointer bg-blue-50 text-blue-600 px-3 py-1.5 rounded-lg font-bold text-xs flex gap-2 items-center">
                      ...
                      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
                        {data.galeria_inicio.map((url, i) => (
                          ...
                          >
                            ...
                            />
                            <div className="absolute bottom-0 inset-x-0 bg-black/60 p-1 flex justify-between items-center backdrop-blur-sm">
                              <button
                                type="button"
                            ...
                            <button
                              ...
                              className="absolute top-1 right-1 bg-red-500 text-white p-1.5 rounded-full shadow-sm"
                            >
                              <X className="w-3 h-3" />
                            </button>
                          </div>
                        ...
                        {newHomeSliderPreviews.map((url, i) => (
                          <div
                            key={`n-${i}`}
                            className="relative aspect-square rounded-xl overflow-hidden border-2 border-green-400 bg-green-50 group"
                          >
                            <Image
                              src={url}
                              alt="New"
                            ...
                            <button
                              ...
                              className="absolute top-1 right-1 bg-red-500 text-white p-1.5 rounded-full shadow-sm z-20"
                            >
                              <X className="w-3 h-3" />
                            </button>
                          </div>
                  ...
                  <section className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
                    <div className="flex justify-between items-center mb-4">
                      ...
                      <button
                        type="button"
                        onClick={addValor}
                        className="bg-blue-50 text-blue-600 px-3 py-1.5 rounded-lg font-bold text-xs flex gap-2 items-center"
                      >
